...
void CompileShaders()
{
	...
	{
		GLchar info[1024];
		glGetShaderInfoLog(ShaderProgram, sizeof(info), NULL, info);
		fprintf(stderr, "Shader Linkage failed: %s\n", info);
		exit(1);
	}

	Matrix_Translation_Location = glGetUniformLocation(ShaderProgram, "TransformMatrix");

	if (Matrix_Translation_Location == -1)
	{
