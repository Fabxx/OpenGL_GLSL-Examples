...
void CompileShaders()
{
	...
	Matrix_Output_Location = glGetUniformLocation(ShaderProgram, "Output");

	if (Matrix_Output_Location == -1)
	{
		fprintf(stderr, "Uniform <Output> not found");
	}

	Matrix_Camera_Location = glGetUniformLocation(ShaderProgram, "Camera");

	if (Matrix_Camera_Location == -1)
	{
		fprintf(stderr, "Uniform <Camera> not found.");
	}
